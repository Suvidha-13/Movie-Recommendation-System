$(function() {
    ...
    $('.movie-button').on('click',function(){
        var my_api_key = 'b8820470e31c4428bd21c99c3c21f732';
        var title = $('.movie').val();
        if (title == "") {
...
// will be invoked when clicking on the recommended movies cards
...
function get_individual_cast(movie_cast,my_api_key) {
    ...
    for(var cast_id in movie_cast.cast_ids) {
        $.ajax({
            type:'GET',
            url:'https://api.themoviedb.org/3/person/' + movie_cast.cast_ids[cast_id] + '?api_key=' + my_api_key, async:false,
